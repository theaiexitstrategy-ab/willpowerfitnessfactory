...
<html><body style="background:#0a0a0a;color:#f2f2f2;font-family:Arial,sans-serif;margin:0;padding:40px 20px;">
  <div style="max-width:560px;margin:0 auto;background:#111;border:1px solid #262626;padding:32px;">
    <h1 style="font-family:'Bebas Neue',Arial,sans-serif;font-size:34px;letter-spacing:0.04em;margin:0 0 8px;">WELCOME TO ${escapeHtml(BRAND.name.toUpperCase())}.</h1>
    <p style="color:#C9A84C;letter-spacing:0.22em;font-size:11px;text-transform:uppercase;margin:0 0 24px;">${escapeHtml(params.businessName)}</p>
  ...
  try {
    await resend.emails.send({
      from: `${BRAND.shortName} <${BRAND.fromEmail}>`,
      to: params.to,
...
export async function sendOrderNotification(params: {
  ...
  try {
    await resend.emails.send({
      ...
      html: `<p>${escapeHtml(params.businessName)} just got a new order.</p>
             <p><strong>${escapeHtml(params.customerName)}</strong> · ${total}</p>
